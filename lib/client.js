...
bedrock.events.on('bedrock-mongodb.ready', function(callback) {
  if(config['truecred-idp']['messages-client'].enable) {
    var interval = 1;
    config['truecred-idp']['messages-client'].clients
      .forEach(function(clientOptions) {
        ...
        api.start(endpoint, interval, function(err) {
          if(err) {
            callback(err);
          }
        });
 ...
 * @param interval 1, 5, or 10 minute intervals to poll the endpoint
 */
