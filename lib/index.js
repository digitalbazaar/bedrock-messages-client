  ...
  async.auto({
    getClient: function(callback) {
      api.get(null, client.id, function(err, record) {
        ...
        pollErrorCount = record.meta.recentPollErrorCount || 0;
        callback(null, record);
      });
    },
    // TODO: Use SSM/HSM to get private key
    getPrivateKey: ['getClient', function(callback) {
    ...
    process: ['poll', function(callback, results) {
      ...
      async.auto({
        emit: callback => {
          async.each(results.poll, (message, callback) => {
            bedrock.events.emit(
              'bedrock-messages-client.message.' + message.type,
              message, callback);
          }, callback);
        },
        store: ['emit', callback => {
          brMessages.store(results.poll, callback);
        }]
      }, (err, results) => {
        callback(err, results.store);
      });
    }]
